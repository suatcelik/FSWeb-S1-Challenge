...
const profilList = profilListesi(fenomenler);
...
function fenomenSil(fenomenler, indeks) {
  ...
  if (indeks >= 0 && indeks < yeniDizi.length) {
...
const enFenomenlerDizi = enFenomenler(fenomenler);
console.log(enFenomenlerDizi);
...
function fenomenGonderimSayisi(fenomenler, profilAdi) {
  for (let i = 0; i < fenomenler.length; i++) {
    ...
    if (fenomen.profile === profilAdi) {
      const gonderiSayisi = fenomen.posts;
...
const willSmithGonderiSayisi = fenomenGonderimSayisi(fenomenler, "Will Smith");
console.log(willSmithGonderiSayisi);
